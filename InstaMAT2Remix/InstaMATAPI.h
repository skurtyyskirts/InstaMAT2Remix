 ...
 *
 * Copyright © 2026 InstaMaterial GmbH - All Rights Reserved.
 *
...
namespace InstaMAT
{
	...
	public:
		...
		/**
		  ...
		  * @param filename The file path of the texture. If null, the layer will be removed.
	...
	{
		GraphVec3F Position; /**< Vertex position. */
	...
	public:
		 ...
		 * Submeshes are used to distinctly identify polygon groups on the mesh.
	...
	public:
		...
		/**
		 * The GraphType enum represents different graph types accessible via the public API.
		 */
	...
	public:
		 ...
		 * @param isAlphaPremultiplied sets aplha as pre-multiplied in the resulting file
		 * @param allowDithering enables dithering when downsampling on export
		 * @return true if successful
		 */
		...
		/**
		 * Swaps the underlying data buffers between both images.
	...
	public:
		 ...
		 * @note This method may only be invoked on the thread that initialized the SDK.
		 ...
		 * 				@note If the output parameter \p variable is owned by a CompositionGraph,
		 ...
		 * @note Currently supported file formats include: OBJ, FBX, ILME
		 *
		 * @param variable The variable.
		 ...
		 * @note This function may only be invoked if the backend context is current on the current thread.
		 * @note Currently supported file formats include: PLY
		 *
		 * @param variable The variable.
		 * @param path The output path.
		 * @return true upon success
		 ...
		 * @note This function may only be invoked if the backend context is current on the current thread.
		 * @note For currently supported file formats @see WriteMeshForGraphVariableToDisk
		 *
		 * @param variable The variable.
		...
		/**
		 * Exports the previously executed instance using the specified \p templateJson to \p outputPathFormat.
		...
		/**
		 * Exports the previously executed instance using the template JSON located at \p path to \p outputPathFormat.
		 * @note Templates can be created using the InstaMAT standalone app.
		 *
		 * @param templateJson A string containing the JSON of the template
		 * @param outputPathFormat The output path format string. The format string supports the same tokens as MAT and Pipeline.
	...
	public:
		 ...
		 * @note The SDK must be fully authorized before the initialization of the SDK can be done.
		 *
		 ...
		 * @return The loaded graph package or nullptr upon failure.
		 */
		 ...
		 * @note This API can cause a lazy graph to be resolved @see IGraph::Resolve for more information.
		 *
		 ...
		 *
		 * @return The used texture memory in bytes.
		 */
		virtual uint64 GetUsedTextureMemoryInBytes() const = 0;
	};
} // namespace InstaMAT

#define INSTAMAT_API_VERSION_MAJOR 3u
...
#define INSTAMAT_API_RELEASEDATE   "2026-02-25"
