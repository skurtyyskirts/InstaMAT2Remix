...
namespace InstaMAT2Remix {
    ...
    void RTXRemixExportNode::SetupMetaDataForGraphObject(IGraphObject& graph) {
        graph.SetMetaDataAsChar(MetaData::KeyAuthor, kPluginAuthor);
    ...
    bool RTXRemixImportNode::GetParameterDefinition(const uint32 index, const IGraph::ParameterType type, Definition& def) {
        if (type == IGraph::ParameterTypeInput) {
             switch(index) {
                 case 0: def.Name="Pull From Remix"; def.Type=IGraphVariable::TypeBoolean; def.ArithmeticValue.BooleanValue=false; return true;
                 case 1: def.Name="Import Textures"; def.Type=IGraphVariable::TypeBoolean; def.ArithmeticValue.BooleanValue=false; return true;
             }
        }
        return false;
    }
    ...
    void RTXRemixImportNode::Execute(const IGraph& elementGraph, const IGraph& entityGraph, IInstaMATGPUCPUBackend& backend) {
        // Pull is zero-prompt and settings-driven (tiling mesh / auto-unwrap come
        // from Settings), so the node exposes a single trigger.
        ArithmeticGraphValue triggerPull;
        if (backend.GetInputParameterConstantValue("Pull From Remix", &triggerPull) && triggerPull.BooleanValue) {
            ...
            if (m_connector) {
                ...
                if (app) {
                    QMetaObject::invokeMethod(
                        app,
                        [connectorGuard]() {
                            if (connectorGuard) connectorGuard->PullFromRemix();
                        },
                        Qt::QueuedConnection);
                }
            }
