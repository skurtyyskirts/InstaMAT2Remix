...
namespace InstaMAT2Remix {
    ...
    void GuiManager::InjectMenu() {
        ...
        // Prefer an existing menu with our title if another plugin (or the host) already created one.
        QMenu* rootMenu = nullptr;
        bool hadExistingRoot = false;
        for (QAction* a : menuBar->actions()) {
            if (!a) continue;
            const QString text = NormalizeActionText(a->text());
            if (text.compare(kPluginName, Qt::CaseInsensitive) == 0 && a->menu()) {
                rootMenu = a->menu();
        ...
        if (!rootMenu) {
            ...
            // If a Help menu exists, place our menu right before it; otherwise append.
            ...
            QMenu* newMenu = new QMenu(kPluginName, menuBar);
            if (helpAction) {
        ...
        // Action labels/order mirror Substance2Remix (WholeBodyCapture) exactly.
        ...
        ensureAction("Duplicate Material to Remix", &GuiManager::onDuplicateMaterial);
    ...
    void GuiManager::onPullFromRemix() {
        ...
        // Zero-prompt Pull (Substance2Remix parity): tiling-mesh and auto-unwrap
        // behavior come from Settings, never from a click-time dialog.
        m_connector->PullFromRemix();
    }
    ...
    void GuiManager::onDuplicateMaterial() {
        if (!m_connector) return;
        m_connector->DuplicateMaterialToRemix();
    }
    ...
    void GuiManager::onAbout() {
        QMessageBox::information(
            GetMainWindow(),
            QString("About %1").arg(kPluginName),
            QString("<b>%1</b><br/>v%2<br/><br/>%3<br/><br/><a href=\"%4\">%4</a>")
                .arg(kPluginName)
                .arg(kPluginVersion)
